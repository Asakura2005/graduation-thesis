...
const AISecurityMonitor = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      showNotification("error", "Không th? t?i d? li?u AI Security");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleUnban = async (userId, username) => {
    if (
      !window.confirm(`B?n có ch?c mu?n g? ban cho user "${username}"?`)
    )
      return;
    ...
    try {
      ...
      showNotification(
        "success",
        `? Ðã g? ban cho ${username} thành công!`,
      );
      fetchData();
    } catch (err) {
  ...
  const handleBan = async (userId) => {
    ...
    try {
      ...
      showNotification(
        "success",
        `?? Ðã ban user thành công! (${banDuration === -1 ? "Vinh vi?n" : `${banDuration} phút`})`,
      );
            ...
            <table className="table table-hover align-middle mb-0">
              <thead>
                <tr>
                  ...
                  <th style={{ fontSize: "0.7rem" }}>Tr?ng thái</th>
